...
for (let index = 0; index < data[0].pokemon.length; index++){
    const singleCardData = data[0].pokemon[index]
    const cards = document.createElement("div");
    cards.className="cards";
    const cardName = document.createElement("h4");
    const cardImg = document.createElement("img");
    cardImg.src=(singleCardData.sprites.front)
    cardName.innerHTML = singleCardData.name;
    cards.append(cardImg,cardName);
    cardContainer.append(cards);

    // console.log(singleCardData.pokemon[0].sprites.front)
        // console.log(singleCardData.pokemon)
...
for(const images of data){
    console.log(images.pokemon[1].sprites)
}
